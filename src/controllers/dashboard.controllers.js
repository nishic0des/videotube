...
const getChannelStats = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (req.user) {
      ...
      const subscribers = subscription.map(
      ...
      if (!videos || videos.length === 0) {
        ...
        const views = videos.map(
          (views) => (totalView = totalView + views.view)
        ); //
    ...
    throw new ApiError(
      500,
      error,
      "Some error occurred while getting the channel stats: Please try again later"
    );
  }
}); //DONE!
