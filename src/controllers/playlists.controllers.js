...
const createPlaylist = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    throw new ApiError(
      500,
      error,
      "An error while creating playlist : try again later"
    );
  }
}); //DONE!
...
const getUserPlaylists = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    throw new ApiError(
      500,
      error,
      "An error while fetching user playlists : try again later"
    );
  }
}); //DONE!
...
const getPlaylistById = asyncHandler(async (req, res) => {
  ...
  if (!isValidObjectId(playlistId)) {
    throw new ApiError(
      404,
      "Inavalid playlist id is provided : enter valid id to get playlist"
    );
  }
  ...
  try {
    ...
    throw new ApiError(
      500,
      error,
      "An error while getting playlist by id : try again later"
    );
  }
}); //Done!
...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
  ...
  if (!(isValidObjectId(playlistId) || isValidObjectId(videoId))) {
    throw new ApiError(
      404,
      "Enter valid playlistId and videoId to add video in playlist"
    );
  }
  ...
  try {
    ...
    throw new ApiError(
      500,
      error,
      "An error occured while adding video in playlist: please try again later"
    );
  }
}); //DONE!
...
const removeVideoFromPlaylist = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!playlistWithoutVideo) {
      throw new ApiError(
        404,
        "Video does not exist in playlist or playlist was not created"
      );
    }

    // console.log(playlistWithoutVideo, "playlistWithoutVideo1")
    // Remove video from the playlist
    const indexOfVideoToBeRemoved =
      playlistWithoutVideo.videos.indexOf(videoId);
    /*
    ...
    throw new ApiError(
      500,
      error,
      "An error occured while removing video from playlist"
    );
  }
}); //DONE!
...
const updatePlaylist = asyncHandler(async (req, res) => {
  ...
  if (!isValidObjectId(playlistId)) {
    throw new ApiError(
      404,
      "Invalid playlistId: Please provide a valid playlistId"
    );
  }
  ...
  try {
    ...
    throw new ApiError(
      500,
      "An error occurred while trying to update the playlist"
    );
  }
}); //DONE!
